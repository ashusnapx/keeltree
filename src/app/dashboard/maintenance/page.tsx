...
import type { MaintenanceItem, SeedData, Vessel } from "@/types";
...
function MaintenanceContent() {
  ...
  const maintenance = vessel.maintenance;
      ...
      >
        ...
        {maintenance.length === 0 ?
          <div className='flex flex-col items-center justify-center py-40 text-center opacity-30'>
            <Wrench className='w-20 h-20 mb-6' />
            <h3 className='text-2xl font-black uppercase tracking-widest'>
              No Work Pending
            </h3>
            <p className='text-sm font-bold uppercase'>
              All systems operational for {vessel.name}
            </p>
          </div>
        : <div className='grid gap-6'>
            {maintenance.map((job: MaintenanceItem) => (
              <motion.div
                key={job.id}
                whileHover={{ x: 10 }}
